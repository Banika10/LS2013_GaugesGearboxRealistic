...
function manualIgnition:update(dt)
	if not self.deactivateMI then
		if self:getIsActive() and self:getIsActiveForSound() and g_gui.currentGuiName == "" and self.isClient then
			if Steerable.manualIgnition.input:getAction('input.startEngine') then
			...
			if Steerable.manualIgnition.input:getAction('input.startEngine') and self.ignitionMode == 2 then
				self.key.mode = 2;
			else		
...
function manualIgnition:updateTick(dt)
	if not self.deactivateMI then
		...
		if self.ignitionMode ~= 2 then
			...
			-- self:onDeactivateAttachments(true);
			self:onDeactivateSounds(true);
			-- self:onDeactivateAttachmentsSounds(true);
			stopAI = true;
		end;
		
		if self.isBroken or self.fuelFillLevel == 0 then
			stopAI = true;
		...
		if stopAI then
			if Hirable.numHirablesHired ~= 0 then
